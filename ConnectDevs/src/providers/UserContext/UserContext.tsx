...
export const UserProvider = ({ children }: iDefaultProviderProps) => {
  ...
  useEffect(() => {
    ...
    if (userToken) {
      const autoLogin = async () => {
        try {
          const response = await api.get<iApiAutoLoginResponseData>(`/users/${userId}`, {
            headers: {
  ...
  const userRegister = async (formData: iRegisterFormData) => {
    try {
      const response = await api.post<iApiUserResponseData>('/register', formData)
      setIsLoading(true)
      ...
      localStorage.setItem('@CONNECTDEVS:USER', JSON.stringify(response.data.user.id))
      localStorage.setItem('@CONNECTDEVS:TOKEN', JSON.stringify(response.data.accessToken))
      navigate('/login')
      ...
      if (typeof errorMessage === 'string' && errorMessage === 'Email already exists') {
  ...
  const userLogin = async (formData: iLoginFormData) => {
    try {
      ...
      if (typeof errorMessage === 'string' && errorMessage === 'Password is too short') {
        toast.error('A senha deve conter pelo menos oito caracteres')

      } else if (typeof errorMessage === 'string' && errorMessage === 'Cannot find user') {
